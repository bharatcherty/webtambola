...
class TicketController extends Controller
{
	...
	{	
		...
		if($data!=null&&$data['status']=="GAMEOVER"){
	...
	public function processTickets()
	{
		...
		if ($dtt > $now||$currentGame->game_status=="GAMEOVER") {
			return false;
		}
		...
		if (count($leftOverArray) > 0) {
			...
			$cladString="";
			foreach($numArray as $na){
				if($na!=""){
					$cladString=$cladString.$na.',';
				}
			}
			...
			foreach ($sheetWiseTickets as $sheets) {
				...
				foreach ($tickets as $ticket) {
					...
					if($sheetIndex<=2){
						array_push($firstHalfSheet, sizeof($matchedArr));
					}else{
						array_push($secondHalfSheet, sizeof($matchedArr));
					}
					$sheetIndex++;
						...
						if (array_key_exists("EARLY5", $claimedPrizeData)) {
							if (sizeof($claimedPrizeData["EARLY5"]) < $avaliablePrizes["EARLY5"] && !in_array($ticket->ticket_number, $claimedPrizeData["EARLY5"])) {
								if (sizeof($matchedArr) >= 5) {
									...
									$data['checked_numbers'] = $cladString;
				...
				if(count($sheetCombo)>0){
					$minSheetCombo = min($sheetCombo);
				}else{
					$minSheetCombo = 0;
				}
				if(count($firstHalfSheet)>0){
					$minFirstHalfCombo = min($firstHalfSheet);
				}else{
					$minFirstHalfCombo =0;
				}
				if(count($secondHalfSheet)>0){
					$minSecondHalfCombo = min($secondHalfSheet);
				}else{
					$minSecondHalfCombo = 0;
				}
				...
				if (count($sheetCombo) == 6&&$sheetType=="FULLSHEET") {
					...
					//First Half sheet bonus in full sheet
					if (array_key_exists("HALFSHEETBONUS", $avaliablePrizes)) {
						if (array_key_exists("HALFSHEETBONUS", $claimedPrizeData)) {
							if (
								sizeof($claimedPrizeData["HALFSHEETBONUS"]) < $avaliablePrizes["HALFSHEETBONUS"]
					...
					//Full sheet bonus for full sheet
